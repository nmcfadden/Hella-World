...
//	#include "LegendScintillationDefinitions.icc"
  ...
  */
  ...
  for (G4int ji=0;ji < numTPB; ji++) {
     ...
     if (LAr_SCPPTPB[ji] > 3.5424*eV){// < 350 nm 
       //Need a definition below 350 eV from Gehmann 
       WLS_emission[ji] = 0.0001;
   ...
   G4double innerR_WLS = innerR_cryo/2-5*micrometer;;//innerR_cryo/2;//0.*m;
...
// wls_LogicalInnerSuface =  new G4LogicalBorderSurface("phy_WLSCylinder_in_surf",physical_fillGas,physical_wls,fWLSoptSurf);
// wls_LogicalOuterSurface =  new G4LogicalBorderSurface("phy_WLSCylinder_out_surf",physical_wls,physical_fillGas,fWLSoptSurf);
